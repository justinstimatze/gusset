...
function App() {
  ...
  // Until the daemon link is up, lead with the walkthrough — a first-time user
  // sees exactly what to do, mirroring `gusset setup`.
  const showOnboarding =
    state !== null && state.connState !== "connected" && !editing;
    ...
    <main className="min-h-[180px] bg-[var(--canvas)] p-4 text-sm text-[var(--ink)]">
      ...
      {state === null ? (
        ...
        <div className="space-y-3">
          {showOnboarding && <Onboarding connState={state.connState} compact />}
          {showForm ? (
            <SettingsForm
              wsUrl={state.wsUrl}
              onSaved={async () => {
