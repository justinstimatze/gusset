...
//   client -> server  { type: "set-name", name }         rename this device
...
export interface PingMsg {
  type: "ping"; // app-level heartbeat; no payload — its arrival is the signal
}
export type ServerMsg = StatusMsg | BeaconMsg | PingMsg;

// Client -> server frames. The first frame is AuthMsg (no discriminant); every
// frame after it is a ClientMsg carrying a `type`.
...
export interface SetNameMsg {
  type: "set-name";
  name: string; // new friendly name for this device
}
export type ClientMsg = PeersMsg | SetNameMsg;
