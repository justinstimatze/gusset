...
import { type ConnState, DaemonClient, IDLE_TIMEOUT_MS } from "./daemon";
...
// Clients are tracked so afterEach can stop them — otherwise each client's
// inactivity watchdog leaves a pending 50s timer dangling after the test.
...
afterEach(() => {
  for (const c of liveClients) c.stop(); // clear watchdog/retry timers
...
describe("DaemonClient", () => {
  ...
  it("forces a reconnect when no frame arrives within the idle timeout", () => {
    vi.useFakeTimers();
    const { client, states } = newClient();
    client.start();
    last(FakeWS.instances).open(); // opened, but the daemon then goes silent

    vi.advanceTimersByTime(IDLE_TIMEOUT_MS); // no frame, not even a heartbeat
    expect(last(states)).toBe("offline"); // watchdog dropped the dead socket

    vi.advanceTimersByTime(500); // first backoff
    expect(FakeWS.instances).toHaveLength(2); // reconnected
  });

  it("a heartbeat resets the watchdog so a live link is not dropped", () => {
    vi.useFakeTimers();
    const { client } = newClient();
    client.start();
    const ws = last(FakeWS.instances);
    ...
    // Deliver a ping just before each expiry, several times over. A live link
  ...
  it("sendName sends a typed set-name frame only when open", () => {
    const { client } = newClient();
    client.start();
    const ws = last(FakeWS.instances);

    expect(client.sendName("laptop")).toBe(false); // not open yet -> refused
    expect(ws.sent).toHaveLength(0);

    ws.open();
    ws.sent.length = 0; // drop the auth frame
    expect(client.sendName("laptop")).toBe(true);
    expect(JSON.parse(ws.sent[0] ?? "")).toEqual({
      type: "set-name",
      name: "laptop",
    });
