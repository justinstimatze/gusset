...
// TestLAN_ObservesCompletedServe proves the "a peer pulled from us" signal fires
// when a peer authenticates, fetches a chunk, and hangs up cleanly — the event a
// one-shot --peer run uses to exit its reachable-back window early.
func TestLAN_ObservesCompletedServe(t *testing.T) {
	id := deriveIdentity(t, "correct horse battery staple lorem ipsum dolor sit")
	served := make(chan net.Addr, 1)
	srv, err := Listen("tcp", "127.0.0.1:0", id, MapSource{"a": []byte("1")},
		WithServeComplete(func(remote net.Addr) { served <- remote }))
	if err != nil {
		t.Fatal(err)
	}
	go func() { _ = srv.Serve() }()
	...
	_ = client.Close() // clean hangup — the peer finished pulling

	select {
	case remote := <-served:
		if remote == nil {
			t.Error("serve-complete fired without a remote address")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("a completed pull was not reported to WithServeComplete")
	}
}

// TestLAN_NoServeCompleteOnFailedHandshake confirms the signal does NOT fire for
// a peer that never authenticates — only a clean, authenticated pull counts.
func TestLAN_NoServeCompleteOnFailedHandshake(t *testing.T) {
	server := deriveIdentity(t, "correct horse battery staple lorem ipsum dolor sit")
	attacker := deriveIdentity(t, "a totally different eight word secret phrase here")
	served := make(chan net.Addr, 1)
	srv, err := Listen("tcp", "127.0.0.1:0", server, MapSource{"x": []byte("secret")},
	...
	select {
	case <-served:
		t.Fatal("serve-complete fired for a peer that never authenticated")
	case <-time.After(300 * time.Millisecond):
		// no signal — correct
	}
}
