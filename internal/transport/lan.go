...
// WithServeComplete installs an observer invoked when a peer finishes a serve
// session cleanly: it authenticated, fetched what it wanted over the connection,
// and hung up. This is the "a peer pulled from us" signal a one-shot sync uses to
// exit its reachable-back window the moment the peer has taken the update, rather
// than lingering the full timeout. Invoked from the per-connection goroutine, so
// it must be safe for concurrent calls.
func WithServeComplete(fn func(net.Addr)) Option {
	return func(s *Server) { s.onServed = fn }
}
...
type Server struct {
	...
	onServed func(net.Addr)  // optional; fired when a peer completes a clean pull
...
func (s *Server) handle(conn net.Conn) {
	...
	// Clean serve session: the peer fetched what it needed and hung up. This is
	// the "peer pulled from us" signal for the reachable-back exit.
	if s.onServed != nil {
		s.onServed(conn.RemoteAddr())
	}
}
