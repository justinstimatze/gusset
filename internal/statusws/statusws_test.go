...
func TestServer_SendsAppLevelHeartbeat(t *testing.T) {
	// Lower the cadence so the test doesn't wait the real 20s.
	old := pingInterval
	...
	readSnapshot(t, ctx, conn) // drain the initial status frame

	// The next non-status frame must be the app-level heartbeat the extension's
	// watchdog relies on — the protocol ping is invisible to a browser, so this
	// visible frame is what proves liveness. ctx carries a 3s deadline, so a
	// missing heartbeat fails the read rather than hanging.
	for {
		var env struct {
			Type string `json:"type"`
		}
